...
class CustomContextMenuListItem extends BaseElement {
  constructor({ $element, onMouseEnter, onMouseLeave }) {
    super({ $element });

    if (onMouseEnter)
      rx.fromEvent(this.getRootElement(), "mouseenter").subscribe((e) =>
        onMouseEnter(e)
      );

    if (onMouseLeave)
      rx.fromEvent(this.getRootElement(), "mouseleave").subscribe((e) =>
        onMouseLeave(e)
      );
  }
}

class CustomContextMenu2 extends BaseElement {
  items = [
    {
  ...
  #initListItems() {
    ...
    const handleMouseEnter = (e, index) => {
      ...
      $submenu.classList.add("irapha-context-menu2__submenu");
      ...
      $submenu.appendChild(item.submenu.children);

      this.getRootElement().appendChild($submenu);
    };

    const handleMouseLeave = (e) => {
      const submenu = this.getRootElement().querySelector(
        ".irapha-context-menu2__submenu"
      );
    };

    const listItems = this.items.map((it, index) => {
      const $div = document.createElement("div");
      $div.classList.add("irapha-context-menu2__list-item");

      this.getRootElement().appendChild($div);

      return new CustomContextMenuListItem({
        $element: $div,
...
new CustomContextMenu2({
  $element: document.querySelector("#test12345"),
});
